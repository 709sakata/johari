...
async function startServer() {
  ...
  app.get("/api/og-image/:id", async (req, res) => {
    ...
    try {
      const scrapDoc = await getDocs(query(collection(db, "scraps")));
      const docData = scrapDoc.docs.find(d => d.id === req.params.id)?.data();
      ...
      const authorName = docData.authorName || "Anonymous";
  ...
  app.get("/api/link-preview", async (req, res) => {
    ...
    try {
      ...
      if (!response.ok) {
        return res.json({ url: targetUrl, title: targetUrl });
      }
