...
export default async function ScrapPage({ params }: PageProps) {
  try {
        ...
        <main className="flex-grow max-w-6xl mx-auto px-4 py-8 w-full">
          <Suspense fallback={
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-gray-500 font-medium font-display uppercase tracking-widest text-[10px]">スレッドを読み込んでいます...</p>
            </div>
          }>
    ...
    console.error('CRITICAL ERROR in ScrapPage:', error);
    return (
      <div className="min-h-screen flex flex-col">
        <Header />
        <main className="flex-grow flex items-center justify-center p-8">
          <div className="text-center bg-white p-12 rounded-[2.5rem] shadow-2xl border border-red-50 max-w-md mx-auto">
            <h1 className="text-2xl font-bold text-gray-900 mb-4">エラーが発生しました</h1>
            <p className="text-gray-500 mb-8">申し訳ありませんが、スレッドの表示中にエラーが発生しました。</p>
            <Link href="/" className="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg">ホームに戻る</Link>
          </div>
        </main>
        <Footer />
      </div>
